...
const levelsData = {
    ...
    1: {
        ...
        lessonText: "בנאי (Constructor) הוא פעולה מיוחדת שיוצרת את העצם ומאתחלת את שדותיו. כאשר שמות הפרמטרים זהים לשמות השדות במחלקה, נשתמש במילת המפתח <code>this</code> (ב-Java וב-C#) כדי להבהיר למחשב שאנו מתכוונים לשדה המחלקה ולא לפרמטר.<br><br>לדוגמה:<pre dir='ltr' class='text-left font-mono bg-slate-900 text-slate-100 p-3 my-2 rounded-xl border border-slate-800 shadow-inner overflow-x-auto text-[11px] font-bold whitespace-pre-wrap'>this.name = name;</pre>",
        badge: "בנאים: this",
    ...
    2: {
        ...
        lessonText: "בנאי מעתיק (Copy Constructor) הוא בנאי שמקבל כפרמטר אובייקט אחר מאותו טיפוס, ומעתיק את כל ערכי תכונותיו לאובייקט החדש שנוצר.<br><br>דוגמה ב-Java:<pre dir='ltr' class='text-left font-mono bg-slate-900 text-slate-100 p-3 my-2 rounded-xl border border-slate-800 shadow-inner overflow-x-auto text-[11px] font-bold whitespace-pre-wrap'>public BeachItem(BeachItem other) {\n    this.name = other.name;\n    this.price = other.price;\n}</pre>בנאי זה מספק העתקה עצמאית (Deep Copy) ללא שיתוף ההפניות.",
    ...
    4: {
        ...
        lessonText: "שרשור בנאים (Delegation) מאפשר לבנאי אחד לזמן בנאי אחר באותה מחלקה כדי למנוע כפל קוד.<br><br>ב-Java נבצע זאת בשורה הראשונה של הבנאי בעזרת:<pre dir='ltr' class='text-left font-mono bg-slate-900 text-slate-100 p-3 my-2 rounded-xl border border-slate-800 shadow-inner overflow-x-auto text-[11px] font-bold whitespace-pre-wrap'>this(...);</pre>ב-C# נשתמש בסימון הבא לפני גוף הבנאי:<pre dir='ltr' class='text-left font-mono bg-slate-900 text-slate-100 p-3 my-2 rounded-xl border border-slate-800 shadow-inner overflow-x-auto text-[11px] font-bold whitespace-pre-wrap'>: this(...)</pre>זימון בנאי אחר חייב להיות הפעולה הראשונה בבנאי!",
    ...
    7: {
        ...
        lessonText: "מכיוון שלא למדנו רשימות דינמיות (כגון List או ArrayList), נשתמש במערך פשוט בגודל קבוע כדי להכיל קבוצת אובייקטים.<br><br>לדוגמה, קיוסק יכול להכיל מערך של פריטי חוף:<pre dir='ltr' class='text-left font-mono bg-slate-900 text-slate-100 p-3 my-2 rounded-xl border border-slate-800 shadow-inner overflow-x-auto text-[11px] font-bold whitespace-pre-wrap'>BeachItem[] items = new BeachItem[5];</pre>",
    ...
    8: {
        ...
        lessonText: "בעת מימוש בנאי מעתיק של מחלקה המכילה אובייקט אחר, אם נבצע העתקה פשוטה של הפניה:<pre dir='ltr' class='text-left font-mono bg-slate-900 text-slate-100 p-3 my-2 rounded-xl border border-slate-800 shadow-inner overflow-x-auto text-[11px] font-bold whitespace-pre-wrap'>this.item = other.item;</pre>שני החפצים יצביעו על אותו אובייקט בזיכרון (Shallow Copy).<br><br>כדי למנוע זאת, עלינו לבצע <b>העתקה עמוקה (Deep Copy)</b> על ידי הפעלת הבנאי המעתיק של האובייקט המוכל.",
    ...
    15: {
        ...
        lessonText: "בעת יצירת עצם ממחלקה יורשת, המחשב חייב קודם כל לבנות את חלק האב שלו. לכן, בשורה הראשונה של בנאי הבן חובה לקרוא לבנאי האב!<br><br>ב-Java נשתמש בקריאה הבאה בשורה הראשונה בגוף הבנאי:<pre dir='ltr' class='text-left font-mono bg-slate-900 text-slate-100 p-3 my-2 rounded-xl border border-slate-800 shadow-inner overflow-x-auto text-[11px] font-bold whitespace-pre-wrap'>super(...);</pre>ב-C# נשתמש בקריאה הבאה לפני פתיחת הסוגריים המסולסלים:<pre dir='ltr' class='text-left font-mono bg-slate-900 text-slate-100 p-3 my-2 rounded-xl border border-slate-800 shadow-inner overflow-x-auto text-[11px] font-bold whitespace-pre-wrap'>: base(...)</pre>",
    ...
    30: {
        ...
        lessonText: "בדומה למחלקת אב, ניתן להגדיר משתנה מטיפוס ממשק שיצביע על אובייקט ממחלקה המממשת אותו.<br><br>לדוגמה:<pre dir='ltr' class='text-left font-mono bg-slate-900 text-slate-100 p-3 my-2 rounded-xl border border-slate-800 shadow-inner overflow-x-auto text-[11px] font-bold whitespace-pre-wrap'>IAiDecisionMaker decider = new SafetyController();</pre>דרך משתנה זה ניתן להפעיל רק את הפעולות שהוגדרו בממשק, והמערכת תפעיל את המימוש המתאים של האובייקט בזמן ריצה.",
